...
#include "../include/newfs.h"  // 使用相对路径
...
{
	/* TODO: 在这里进行挂载 */
	if (my_mount(newfs_options) != NEWFS_ERROR_NONE)
	{
		do
		{
			printf("SFS_DBG:%s mount error\n", __func__);
		} while (0);
...
{
	/* TODO: 在这里进行卸载 */
	if (my_umount() != NEWFS_ERROR_NONE)
	{
		do
		{
			printf("SFS_DBG:%s unmount error\n", __func__);
		} while (0);
		fuse_exit(fuse_get_context()->fuse);
		return;
	}
...
int newfs_getattr(const char *path, struct stat *newfs_stat)
{
	...
	dentry= (struct newfs_dentry*)((uintptr_t)newfs_lookup(path, &is_find, &is_root));
	//printf("NEWFS_DBG: received dentry = %p\n", dentry);

	//printf("Enum value: %d\n",dentry->inode->dentry->ftype);
	if (is_find == FALSE)
	{
...
int main(int argc, char **argv)
{
	...
	newfs_options.device = strdup("/home/students/220110707/ddriver");
